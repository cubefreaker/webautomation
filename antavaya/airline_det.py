...
from objects._airline_det import InvAirline
from objects._create_inv import InvCreate
from objects._check_journal_prev import CheckPrev
import names
import datetime
import random
...
class InvoicingAirline(unittest.TestCase):
    def setUp(self):
        ...
        self.base_url = "http://antavaya.opsifin.com/"
        self.verificationErrors = []
        self.accept_next_alert = True

    def test_invoicing_airline(self):
        driver = self.driver
        url = "https://antavaya.opsifin.com/"
        driver.get(url)
        time.sleep(1)
        driver.get(url)
        ...
        def run():
            ...
            time.sleep(0.5)
            inv.customer('a. d')
            # inv.customer(random.choice(['Retail Ho','Alfa']))
            inv.pax_type('Adult')
            inv.pax_f_name()
            inv.pax_l_name()
            inv.pax_email()
            ...
            inv.supplier(random.choice(['BSP / IATA DOMESTIC', 'LION AIRLINES', 'GARUDA']))
            inv.flight_type()
            inv.pnr_code()
            inv.ticket_no()
            inv.ticketed_date()
