...
from objects._train_det import InvTrain
import names
import datetime
...
class InvoicingTrain(unittest.TestCase):
    ...
    def test_invoicing_train(self):
        ...
        def run():
            inv.add()
            time.sleep(0.5)
            inv.customer(random.choice(['unilever i', 'unilever o', 'unilever t']))
            inv.pax_type('Adult')
            inv.pax_f_name()
            inv.pax_l_name()
            inv.pax_email()
            ...
            inv.supplier(random.choice(['RAILWORLD', 'ACE', 'KERETA']))
            inv.pnr_code()
            inv.ticket_no()
            inv.ticketed_date()
            inv.issuer()
